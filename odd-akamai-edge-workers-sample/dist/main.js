...
logger.log("in server.js"); // Main EdgeWorker event handler
...
async function onClientRequest(request) {
  try {
    ...
    const visitorState = lodash.get(targetResponse, "visitorState");

    if (response.headers.get('Content-Type')?.includes('text/html')) {
      const body = await response.text(); // Modify the body to include the script code

      const modifiedBody = body.replace('</head>', `
      <script>
        const visitorState = ${visitorState};
      </script>
    </head>`); //request.respondWith(createResponse(STATUS, { "Content-Type": "text/html" }, JSON.stringify(modifiedBody)));
      //request.respondWith(createResponse(STATUS, { headers: HEADERS }, "hello1"));
      //return createResponse(STATUS, HEADERS, "hello1");

      request.respondWith(200, {}, '<html><body><h1>Target response activity</h1></body></html>');
    } else {
      ...
      request.respondWith(200, {}, '<html><body><h1>Target response activity 2</h1></body></html>');
    }

    logger.log("Sending response", JSON.stringify(targetResponse)); // Sending the response directly back to the client
    //request.respondWith(createResponse(STATUS, { headers: HEADERS }, JSON.stringify(targetResponse)));
    //request.respondWith(createResponse(STATUS, { headers: HEADERS }, "hello3"));

    request.respondWith(200, {}, '<html><body><h1>Target response activity 3</h1></body></html>');
  } catch (error) {
    logger.error("Error processing the request: " + error.message); //request.respondWith(createResponse(500, { headers: { "Content-Type": "text/plain" } }, "Internal Server Error"));

    request.respondWith(200, {}, error.message);
  }
